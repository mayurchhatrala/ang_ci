...
class FUpload {
    ...
    private function _fConfigValue() {
        try {
             ...
             * CONVER INTO BYTES...
             */
            $this->fConfig['max_size'] = 2048 * (int) $this->fLimit;
    ...
    private function _fToJPG() {
        try {
            ...
            foreach ($this->fUploaded as $K => $V) {

                $fileName = $V;

                $fSRC = $this->fBasePath . $V;
                $fInfo = getimagesize($fSRC);

                list($fWidth, $fHeight) = getimagesize($fSRC);
                $size = $fWidth > $fHeight ? $fWidth : $fHeight;

                $this->fUploaded[$K] = $fileName;

                /*
                 *  ADD WHITE SPACE
                 *  IF USER NOT NEED TRANPARENT IMAGE
                 */
                if ($this->fTrans) {

                    switch ($fInfo['mime']) {
                        case 'image/jpeg':
                            $IMAGE = imagecreatefromjpeg($fSRC);
                            break;

                        case 'image/png':
                            $IMAGE = imagecreatefrompng($fSRC);
                            break;

                        case 'image/gif':
                            $IMAGE = imagecreatefromgif($fSRC);
                            break;
                    }
                    unlink($fSRC);
                     ...
                     */
                    imagejpeg($IMAGE, $this->fBasePath . $fileName, $this->fCompressQuality);
                    imagedestroy($IMAGE);

                    if ($fWidth < 960 && $fHeight < 1704) {

                        $im = @imagecreate($size, $size);
                        $bgColor = imagecolorallocate($im, 255, 255, 255);
                        imagejpeg($im, $this->fBasePath . 'white.png', $this->fCompressQuality);
                        imagedestroy($im);

                        $src = imagecreatefromjpeg($this->fBasePath . $fileName);
                        $dest = imagecreatefromjpeg($this->fBasePath . 'white.png');

                        imagealphablending($dest, false);
                        imagesavealpha($dest, true);

                        if ($fWidth > $fHeight)
                            imagecopymerge($dest, $src, 0, (round(($size - $fHeight) / 2)), 0, 0, $fWidth, $fHeight, 100);
                        else
                            imagecopymerge($dest, $src, (round(($size - $fWidth) / 2)), 0, 0, 0, $fWidth, $fHeight, 100);

                        imagejpeg($dest, $this->fBasePath . $fileName, $this->fCompressQuality);
                        imagedestroy($dest);
                        imagedestroy($src);
                    }
                }
    ...
    private function _fThumbsUp() {
        try {
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                $rqstRatio = $this->defaultRatio;
                $isCustom = FALSE;
                $customRatio = array();
                if ((isset($this->fWidth) && !empty($this->fWidth)) && ((isset($this->fHeight) && !empty($this->fHeight)))) {
                    $rqstRatio = $this->fWidth;
                    $isCustom = TRUE;
                }

                for ($k = 0; $k < count($rqstRatio); $k++) {
                    if ($isCustom) {
                        $customRatio = array(
                            'W' => $this->fWidth[$k],
                            'H' => $this->fHeight[$k]
                    ...
                    $this->_createThumbNail($rqstRatio[$k], $this->fUploaded[$i], $fPath, $isCustom, $customRatio);
                }
            }
        } catch (Exception $ex) {
    ...
    /*
     * TO CREATE THE THUMBNAIL OF THE UPLOADED IMAGE...
     */

    private function _createThumbNail($currRatio, $fImage, $fPath, $isCustom = FALSE, $customRatio = array()) {
        try {
            $fThumbPath = $this->fBasePath . 'thumb/' . $currRatio . '/' . $fImage;
            ...
            if (!is_dir($this->fBasePath . 'thumb/' . $currRatio . '/'))
                mkdir($this->fBasePath . 'thumb/' . $currRatio . '/', 0777, TRUE);

            /*
             * FETCH THE HEIGHT AND WIDTH OF THE UPLOADED IMAGE
             */
            $imgList = list($fW, $fH) = getimagesize($fPath);
            $imgRatio = $currRatio / min($fW, $fH);

            $fSize = array(
                'W' => $isCustom ? $customRatio['W'] : $fW * $imgRatio,
                'H' => $isCustom ? $customRatio['H'] : $fH * $imgRatio
            );
             ...
             * 2.0.3.1
             */
            $this->_fInitThumbConfig($fPath, $fThumbPath, $fSize, !$isCustom);
    ...
    private function _fInitThumbConfig($fPath, $fThumbPath, $fSize, $maintainRaio = TRUE) {
        try {
            if ($fPath !== '' && $fThumbPath !== '' && !empty($fSize)) {
                $this->fThumbConfig = array(
                    'image_library' => 'gd2',
                    'maintain_ratio' => $maintainRaio,
                    'create_thumb' => FALSE,
                    'source_image' => $fPath,
    ...
    function fDelete($targetFile = '') {
        try {
            if ($targetFile != '') {
                /*
                 * REMOVE SELECTED ONE OR MORE FILE...
                 */
                if (is_string($targetFile))
                    $targetFile = explode(',', $targetFile);

                $this->_deleteFiles($targetFile);
            } else {
                /*
                 * REMOVE ALL THE FILES OF THE TARGETED FOLDER...
                 */
            }
        } catch (Exception $ex) {
    ...
    private function _deleteFiles($targetFile = array()) {
        try {
            if (!empty($targetFile)) {
                /*
                 * REMOVE FROM THE BASE FOLDER...
                 */
                $fBasePath = $this->fBasePath;
                foreach ($targetFile as $file) {
                    unlink($fBasePath . $file);
                }

                /*
                 * REMOVE FROM THE THUMBNAIL FOLDER
                 */
                foreach ($this->defaultRatio as $ratio) {
                    $fBasePath = $this->fBasePath . 'thumb/' . $ratio . '/';
                    foreach ($targetFile as $file) {
                        unlink($fBasePath . $file);
                    }
                }

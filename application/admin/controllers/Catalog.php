...
class Catalog extends CI_Controller {
    ...
    public function formData() {
        try {
             ...
             */
            $response = $this->catalog_model->getCatalogRecord($request->requestId, true);
            //_print_r($response);
            if (!empty($response)) {
                $this->RESP = $response;
            } else {
    ...
    public function CatalogRecord() {
        try {
             ...
             * TO CHECK THAT USERNAME AND PASSWORD ARE CORRECT OR NOT
             */
            $response = $this->catalog_model->getCatalogRecord();
            //_print_r($response);
            if (!empty($response)) {
                $this->RESP = $response;
            } else {
    ...
    private function _addRecord($requestFor) {
        try {
            if ($requestFor !== '') {
                 ...
                 */
                return $this->catalog_model->addRecord($requestFor, $request);
            } return false;
        } catch (Exception $ex) {
    ...
    private function _deleteRecord($requestFor, $requestId) {
        try {
            if ($requestFor !== '' && $requestId !== '') {
                return $this->catalog_model->deleteRecord($requestFor, $requestId);
            } return false;
        } catch (Exception $ex) {
    ...
    private function _updateRecord($requestFor, $requestId) {
        try {
            if ($requestFor !== '' && $requestId !== '') {
                 ...
                 * SAVE IN THE DATABASE
                 */
                return $this->catalog_model->updateRecord($requestFor, $request, $requestId);
            } return false;
        } catch (Exception $ex) {
    ...
    private function _activeRecord($requestFor, $requestId) {
        try {
            if ($requestFor !== '' && $requestId !== '') {
                 ...
                 * SAVE IN THE DATABASE
                 */
                return $this->catalog_model->activeRecord($requestFor, $postdata, $requestId);
            } return false;
        } catch (Exception $ex) {
            throw new Exception('Crud Controller : Error in _updateRecord function - ' . $ex);
        }

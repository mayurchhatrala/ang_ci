...
class Retailer_Model extends CI_Model {
    ...
    public function getRetailerRecord($requestId = '', $isEdit = FALSE) {
        ...
        try {
            ...
            $fields[] = 'r.vRetailerstreetName AS vRetailerstreetName';
            $fields[] = 'r.vRetailerstreetNumber AS vRetailerstreetNumber';
            $fields[] = 'r.vRetailerAddressLine AS vRetailerAddressLine';
            $fields[] = 'r.vRetailerCity AS vRetailerCity';
            $fields[] = 'r.vRetailerDistrict AS vRetailerDistrict';
            ...
            $qry = 'SELECT ' . $fields . ' FROM ' . $tbl . ' AS r JOIN db_category as c ON c.iCategoryID = r.iCategoryID' . $condition . '';
    ...
    function addRecord($requestFor, $requestObject = array()) {
        try {
            if ($requestFor !== '' && !empty($requestObject)) {
                ...
                if (isset($insData['vRetailerstreetName']) && $insData['vRetailerstreetName'] == '' || $insData['vRetailerstreetName'] == 'undefined') {
                    $insData['vRetailerstreetName'] = '';
                }
                if (isset($insData['vRetailerstreetNumber']) && $insData['vRetailerstreetNumber'] == '' || $insData['vRetailerstreetNumber'] == 'undefined') {
                    $insData['vRetailerstreetNumber'] = '';
                }
                if (isset($insData['vRetailerCity']) && $insData['vRetailerCity'] == '' || $insData['vRetailerCity'] == 'undefined') {
                    $insData['vRetailerCity'] = '';
                }
                if (isset($insData['vRetailerDistrict']) && $insData['vRetailerDistrict'] == '' || $insData['vRetailerDistrict'] == 'undefined') {
                    $insData['vRetailerDistrict'] = '';
                }
                if (isset($insData['vRetailerCountry']) && $insData['vRetailerCountry'] == '' || $insData['vRetailerCountry'] == 'undefined') {
                    $insData['vRetailerCountry'] = '';
                }
                if (isset($insData['vRetailerState']) && $insData['vRetailerState'] == '' || $insData['vRetailerState'] == 'undefined') {
                    $insData['vRetailerState'] = '';
                }
                ...
                if ($insId > 0) {
                    ...
                    if (isset($_FILES) && !empty($_FILES)) {
                        ...
                        $PARAM = array(
                            ...
                            'fThumb' => TRUE,
                            'fCopyText' => FALSE,
                            'fWidth' => array(292, 384),
                            'fHeight' => array(126, 189),
                            'fTrans' => FALSE
                        );
        ...
        try {
            if ($requestFor !== '' && !empty($requestObject) && $requestId !== '') {
                ...
                if (isset($updateData['vRetailerLogo']) && $updateData['vRetailerLogo'] != 'undefined') {
                    $updateData['vRetailerLogo'] = '';
                }
                
                if (isset($updateData['vRetailerstreetName']) && $updateData['vRetailerstreetName'] == '' || $updateData['vRetailerstreetName'] == 'undefined') {
                    $updateData['vRetailerstreetName'] = '';
                }
                if (isset($updateData['vRetailerstreetNumber']) && $updateData['vRetailerstreetNumber'] == '' || $updateData['vRetailerstreetNumber'] == 'undefined') {
                    $updateData['vRetailerstreetNumber'] = '';
                }
                if (isset($updateData['vRetailerCity']) && $updateData['vRetailerCity'] == '' || $updateData['vRetailerCity'] == 'undefined') {
                    $updateData['vRetailerCity'] = '';
                }
                if (isset($updateData['vRetailerDistrict']) && $updateData['vRetailerDistrict'] == '' || $updateData['vRetailerDistrict'] == 'undefined') {
                    $updateData['vRetailerDistrict'] = '';
                }
                if (isset($updateData['vRetailerCountry']) && $updateData['vRetailerCountry'] == '' || $updateData['vRetailerCountry'] == 'undefined') {
                    $updateData['vRetailerCountry'] = '';
                }
                if (isset($updateData['vRetailerState']) && $updateData['vRetailerState'] == '' || $updateData['vRetailerState'] == 'undefined') {
                    $updateData['vRetailerState'] = '';
                }
                ...
                if (!isset($updateData['vRetailerLogo']) || $updateData['vRetailerLogo'] !== 'undefined') {
                    ...
                    if (isset($_FILES) && !empty($_FILES)) {
                        ...
                        if ($updateFileData) {
                            ...
                            // to remove old image
                            if (isset($updatedRec['vProfilePic'])) {
                    ...
                    // GET IMAGE AND UPDATE IMAGE WHICH LAST UPDTEED OR INSERTED
                    $query = $this->db->query("SELECT " . $this->tblImage . " FROM " . $this->tbl . " WHERE " . $this->tblKey . " = " . $requestId);
                    $updatedRec = $query->row_array();
                    $this->db->update($this->tbl, $updateData, array($this->tblKey => $requestId));

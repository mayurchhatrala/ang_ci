...
class Key extends REST_Controller {
    ...
    public function index_post() {
        ...
        if (self::_insert_key($key, array('level' => $level, 'ignore_limits' => $ignore_limits))) {
            $this->response(array('STATUS' => 200, 'KEY' => $key), 201); // 201 = Created
        } else {
            $this->response(array('STATUS' => 101, 'ERROR' => 'Could not save the key.'), 500); // 500 = Internal Server Error
        }
    }
    ...
    public function index_delete() {
        ...
        if (!self::_key_exists($key)) {
            // NOOOOOOOOO!
            $this->response(array('STATUS' =>101, 'ERROR' => 'Invalid API Key.'), 400);
        }
